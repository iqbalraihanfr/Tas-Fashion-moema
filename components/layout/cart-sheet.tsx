    ...
    <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
      <SheetContent side="right" className="w-full sm:max-w-md flex flex-col p-0">
        <SheetHeader className="p-6 border-b border-border">
          <SheetTitle className="text-lg font-medium uppercase tracking-wide">Shopping Bag ({items.length})</SheetTitle>
        </SheetHeader>
            ...
            items.map((item) => (
              <div key={`${item.id}-${item.color}`} className="flex gap-4">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <h4 className="text-sm font-medium uppercase tracking-wide line-clamp-1">{item.name}</h4>
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-border space-y-4 bg-background">
             ...
             <div className="grid gap-3">
               <Button size="lg" className="w-full uppercase tracking-widest text-xs rounded-none" asChild onClick={() => setIsCartOpen(false)}>
                  <Link href="/checkout">Checkout</Link>
               </Button>
               <Button variant="outline" size="lg" className="w-full uppercase tracking-widest text-xs rounded-none" asChild onClick={() => setIsCartOpen(false)}>
