...
import { cleanupQaShowcaseByTitle } from "./helpers/qa-cleanup";
...
test.describe("Admin Showcase Smoke", () => {
  ...
  test("admin can create, edit, and delete a category showcase", async ({ page }) => {
    ...
    try {
      ...
      await expect(page).toHaveURL(/\/admin\/dashboard\/showcase$/, { timeout: 20000 });
      ...
      await cleanupQaShowcaseByTitle(showcaseTitle);
    }
  });
});
